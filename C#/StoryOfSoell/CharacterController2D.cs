...
public class CharacterController2D : MonoBehaviour
{
	...
	[SerializeField] private Collider2D m_CrouchDisableCollider;				// A collider that gets disabled when crouching
	...
	public void Move(float move, bool crouch, bool jump)
	{
		...
		if (m_Grounded || m_AirControl)
		{
			if (crouch)
			{
				...
				// Disable one collider when crouching
			...
			// If player is moving right while facing left
			if (move > 0 && !m_FacingRight)
			{
				Flip();
			}
			// If player is moving left while facing right
