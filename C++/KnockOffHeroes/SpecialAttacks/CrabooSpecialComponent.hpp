/*
studies: MultiMediaTechnology / FHS
cause: MultiMediaProjekt 2a
team: Alija Suljic, Sabine Steiner, Markus Rinnerthaler
*/

#pragma once
#include "ISpecialAttackComponent.hpp"

namespace mmt_gd
{
class CrabooSpecialComponent final : public ISpecialAttackComponent
{
public:
    CrabooSpecialComponent(GameObject& gameObject, GameObject& clawOne, GameObject& clawTwo);

    bool init() override;
    void update(float deltaTime) override;
    void specialAttack() override;
    void stopSpecial() override;

private:
    GameObject& m_clawOne;
    GameObject& m_clawTwo;
    const float m_heightDisplacement;
    float       m_specialTimer;
    const float       m_specialTimerLimit;
    bool        m_specialLaunched;
};
} // namespace mmt_gd